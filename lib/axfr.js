...
const digestLengths = {
  1: 20, // SHA-1 RFC3658
  2: 32, // SHA-256 RFC4509
  3: 32, // GOST R 34.11-94 RFC5933
  4: 48, // SHA-384 RFC6605
};
  ...
  async sendAXFR(req, rinfo) {
    ...
    while (await iter.next()) {
      ...
      for (const rr of zone) {
        // Ignore DS records with invalid digests
        if (rr.type === types.DS) {
          const isValidDigest = digestLengths[rr.data.digestType] &&
            digestLengths[rr.data.digestType] === rr.data.digest.length;

          if (!isValidDigest) {
            this.logger.warning(`DS for ${rr.name} has an invalid digest - skipping`);
            continue
          }
        }
