...
class Plugin {
  ...
  async sendAXFR(req, rinfo) {
    ...
    if (!this.ipList.includes(address)) {
      this.logger.debug("Address %s cannot send zone transfer request check allowed ip addresses.", address);
      return this.refuse();
    }
    ...
    const iter = this.chain.db.txn.iterator(true);
    let progress = 0;
    ...
    while (await iter.next()) {
      ...
      for (const rr of dns.records()) {
        if (rr.type === types.RRSIG) continue;
